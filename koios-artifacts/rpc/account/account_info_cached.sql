CREATE OR REPLACE FUNCTION {{SCHEMA}}.account_info_cached(_stake_addresses text [])
RETURNS TABLE (
...
BEGIN
  ...
  RETURN QUERY
    ...
    FROM
      {{SCHEMA}}.stake_distribution_cache AS sdc
      LEFT JOIN (
        SELECT
          sas.id,
          sas.view,
...
COMMENT ON FUNCTION {{SCHEMA}}.account_info IS 'Get the cached account information for given stake addresses'; -- noqa: LT01
