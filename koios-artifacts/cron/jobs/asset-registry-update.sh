...
last_commit="$(psql ${DB_NAME} -c "select last_value from {{SCHEMA}}.control_table where key='asset_registry_commit'" -t | xargs)"
...
psql ${DB_NAME} -qb -c "INSERT INTO {{SCHEMA}}.control_table (key, last_value) VALUES ('asset_registry_commit','${latest_commit}') ON CONFLICT(key) DO UPDATE SET last_value='${latest_commit}'"
