...
[[ $(psql ${DB_NAME} -qbt -c "SELECT {{SCHEMA}}.active_stake_cache_update_check();" | tail -2 | tr -cd '[:alnum:]') != 't' ]] &&
...
psql ${DB_NAME} -qbt -c "SELECT {{SCHEMA}}.active_stake_cache_update(${db_last_epoch_no});" 1>/dev/null 2>&1
