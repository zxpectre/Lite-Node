...
DROP VIEW IF EXISTS {{SCHEMA}}v0.account_list;

CREATE VIEW {{SCHEMA}}v0.account_list AS
SELECT stake_address.view AS id
FROM
  stake_address;

DROP VIEW IF EXISTS {{SCHEMA}}v0.asset_list;

CREATE VIEW {{SCHEMA}}v0.asset_list AS
SELECT
...
DROP VIEW IF EXISTS {{SCHEMA}}v0.asset_token_registry;

CREATE VIEW {{SCHEMA}}v0.asset_token_registry AS
SELECT
  ...
  logo
FROM
  {{SCHEMA}}.asset_registry_cache;

DROP VIEW IF EXISTS {{SCHEMA}}v0.blocks;

CREATE VIEW {{SCHEMA}}v0.blocks AS
SELECT
